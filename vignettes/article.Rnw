...
\begin{enumerate}
\item Nicely format
\item Make sure all files are available and run \fct{devtools::document} and stuff
\item Find out if people are interested in these topics and make suggestions for ``workshop'' (e.g., more-indetph on package dev [vignette], bash, git, GitHub, \LaTeX (and tables), ...)
\end{enumerate}
...
\item But: There are good and bad practices (e.g., stuff like \code{stwd("~/foo/bar")}, \code{read.csv("~/foo/bar.csv")})
...
\end{Verbatim}
%
\marginpar{The \code{.Rproj} file is just is RStudio's ``project file'', storing some settings regarding your project organization. In particular it opens a new RStudio session in that current folder...}
We get the minimal file and folder structure of an \proglang{R}-package (where \code{Rpackage.Rproj} is optional if you are using RStudio).

\leftmarginpar{\fct{usethis::use\_data\_raw}}
The \code{data-raw} folder is where the untouched raw data goes. But also all the scripts that manipulate this raw data and generate some other output (e.g., data for the analysis). I usually put all my scripts there since it allows me to easily store intermediate \code{objects}, attach them to the package and use them at a later stage (e.g., a model fit as returned by \fct{fit}, see below). Let's get concrete
...
You will run this function a lot! It essentially simulates a \code{library(Rpackage)} call (i.e., loads the package is it was installed).
<<load-all>>=
devtools::load_all()
...
@
%
\marginpar{By the way, you can put other objects (other than functions) in a script in the \code{R} folder... Remember, the scripts get just sourced during package loading (try it out and put \code{print("I'm getting sourced")}).}
Again, ``test it''
<<eval=FALSE>>=
devtools::load_all()
foo()
...
\pkg{roxygen2} only generates documentation from the source in the \code{R} directory (at least per default). I usually create a file \code{R/roxygen.R} where I put all the documentation stuff, with no corresponding source file in the \code{R} directory, such as our \code{useful\_data} data. So, let's document it
...
%
% What is reproducible research? Anyone should be able to generate your results on the fly! Now imagine that you want to present the results of our regression analysis which lives in \code{Rpackage::analysis\_script\$fit}. You maybe copy paste the parameter estimates to an excel file or use some tool to build a \LaTeX table. In my view, this introduces three problems
% %
% \begin{enumerate}
...
% \end{enumerate}
% %
% \marginpar{But how do I collaborate on a paper? I would recommend that you only collaborate with people familiar to this workflow (and \proglang{git})! ;)}
%
% %% Sometimes it is fun...
% \begin{tabular}{lll}
...
%
% \marginpar{You can also use \pkg{knitr} and \code{.Rmd} files to generate vignettes or papers, but I am quite old-fashioned and prefer \code{Swevae} (i.e., \code{.Rnw}) \url{https://stat.ethz.ch/R-manual/R-devel/library/utils/doc/Sweave.pdf}.}
...
<<install, eval=FALSE>>=
install.packages("../Rpackage.tar.gz")
@
...
\item Please, please, please learn some \proglang{git} for versioning (and necessarily some \proglang{bash})
\end{itemize}
...
\begin{itemize}
\item \citeauthor{Wickham+Bryan:2023}'s book on package development
\item \proglang{bash} \citep{Bash:2019}: \url{https://linuxcommand.org/lc3_learning_the_shell.php}
\item For windows users: download \proglang{Git BASH} at \url{https://gitforwindows.org/}
\item \proglang{git}: The first three chapters of \citet{Git:2014}
\end{itemize}
