...
\item Make sure this is a good example, run \fct{devtools::check} and fill in the \code{DESCRIPTION} file, ...
...
We get the minimal file and folder structure of an \proglang{R}-package (\code{Rpackage.Rproj} is optional if you are using RStudio).
...
You will run \fct{devtools::load\_all} a lot! It essentially simulates a \code{library(Rpackage)} call (i.e., loads the package is it was installed).
<<load-all>>=
devtools::load_all()
